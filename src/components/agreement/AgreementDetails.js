...
import { Accordion, AccordionDetails, Paper, AccordionSummary, Alert, Avatar, AvatarGroup, Badge, Box, Button, CardContent, Grid, Table, TableBody, TableContainer, TableHead, TableRow, TextField, Typography, CircularProgress } from '@mui/material'
import { bold_name, btn_connect, btn_hire, card, circularprog, circularprog_hire, df_jc_ac, ptag } from '../../theme/CssMy'
...
import { endContract, fundMilestone, grantRefundRequest, payMilestone, raiseRefundRequest, requestPayment, updateRefundRequest } from '../../services/agreement'
...
export default function AgreementDetails() {
    ...
    useEffect(() => {
        const func = async () => {
            setU1((await getExpertById(agreement.user1)).data)
            setU2((await getExpertById(agreement.user2)).data)
            console.log(u1, u2)
        }
        func()
    }, [])
        ...
        <>
            {u1 && u2 && <>
                ...
                <Box sx={{ display: 'flex', padding: { md: '2% 20%', xs: '2%' }, flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    {u1._id === (JSON.parse(localStorage.getItem('ybUser')))._id && isAllPaymentResolved && <Box sx={df_jc_ac}>
